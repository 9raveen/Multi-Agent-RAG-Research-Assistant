...
flowchart TD
    ...
    SCROLL --> MAPREDUCE[Map-Reduce Summarization<br/>GPT OSS 120B<br/>batches summarized, then combined]
    MAPREDUCE --> DONE
    DETECT -->|No| RESEARCH[Research Agent<br/>Retrieves top-k chunks from Qdrant<br/>Scoped to document + user_id]
    RESEARCH --> SYNTH[Synthesis Agent<br/>GPT OSS 120B<br/>Answers using ONLY retrieved context]
...
| **Synthesis**     | **GPT OSS 120B** (via Groq)            | Generates a thorough answer using _only_ the retrieved context — instructed to say "not found" rather than hallucinate                                     |
...
| **Summarization** | **GPT OSS 120B** (via Groq)            | Map-reduce whole-document summary — bypasses top-k similarity search entirely (see [dedicated section](#document-summarization-map-reduce))                |
...
| LLM (synthesis + summarization) | **Groq** `openai/gpt-oss-120b`                          | Reasoning model with `reasoning_effort=low` — strong grounded synthesis and map-reduce summarization via Groq's OpenAI-compatible endpoint                |
| LLM (critique)                  | **Groq** `llama-3.1-8b-instant`                         | Cheaper/faster for a binary pass/fail classification task — synthesis quality doesn't require large-scale reasoning here                                   |
...
| **Follow-up messages failing with `ERR_HTTP2_PROTOCOL_ERROR`**                                       | `asyncio.run()` in `crud.py`'s `run_sync()` helper creates a new event loop and sets it as the thread-local loop. On HTTP/2 keep-alive connections Uvicorn reuses the same OS thread across requests — the second `asyncio.run()` call from the streaming generator collided with the residual loop state from the first request, aborting the stream | Replaced `asyncio.run()` with a dedicated `ThreadPoolExecutor` — each DB call runs in its own isolated worker thread with a fresh event loop that is completely decoupled from Uvicorn's loop, making follow-up messages safe on HTTP/2                                                                         |
| **Cross-user document data leak — other users' files returned in queries**                           | Two compounding bugs: (1) documents uploaded before `user_id` tagging was enforced had no `user_id` field in Qdrant — Qdrant's `FieldCondition` filter silently passes points where the filtered field is absent, so every query returned those orphaned points to every user. (2) `chunk_id_to_point_id()` hashed only `chunk_id` (derived from filename + page + index) — two users uploading the same filename produced identical point IDs, and Qdrant's upsert silently overwrote User A's vectors with User B's | (1) Purged all 121 untagged orphan points from Qdrant via a targeted scroll-then-delete script. (2) Made `user_id` mandatory in both `embedder.py` and `retriever.py` — both raise immediately if not provided (fail-closed, not fail-open). (3) Point IDs now hash `user_id + chunk_id` together so different users' uploads of the same filename produce independent, non-colliding points |
